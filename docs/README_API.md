...
El payload exige `pagina`, `principalKey`, `reportDate` y `requestKey`. La misma
`requestKey` devuelve la corrida existente; otra clave permite una nueva corrida
auditable para la misma plataforma, principal y fecha.
