...
create or replace function public.purge_mastercrm_technical_history_v1(
  p_cutoff_date date
)
returns table (
  cutoff_date date,
  report_daily_snapshots_deleted integer,
...
begin
  ...
  with deleted as (
    delete from public.report_daily_snapshots
    where report_date < v_cutoff_date
    returning 1
  )
  select count(*)::integer
    into report_daily_snapshots_deleted
  from deleted;
